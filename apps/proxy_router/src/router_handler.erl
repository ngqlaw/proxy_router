...
-spec(handle_info(Info :: timeout() | term(), State :: #state{}) ->
    ...
    {noreply, NewState :: #state{}, timeout() | hibernate} |
    {stop, Reason :: term(), NewState :: #state{}}).
handle_info({'DOWN', Ref, process, _Object, _Reason}, #state{
        monitor_ref = Ref,
        shutdown = 0, 
        close_timer = Old
    } = State) ->
...
handle_info({'DOWN', Ref, process, _Object, Reason}, #state{
    ...
    case is_reference(Old) andalso (false == erlang:read_timer(Old)) of
        ...
        false -> 
            ...
            {noreply, State#state{close_timer = CloseTimer, close_ref = CloseRef}}
    end;
handle_info({stop, CloseRef, _Reason}, #state{close_ref = CloseRef} = State) ->
    {stop, normal, State};
